...
int main(){
    ...
    denied=e;denied.command->cam|=1;n=encode_envelope(denied,body,std::nullopt,out);assert(n);assert(!registry.validate(Bytes{out}.first(*n),budget));assert(fixture.validations==before);
