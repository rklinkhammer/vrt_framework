...
int main(){
 ...
 for(std::size_t expected:{1u,2u,1023u,1024u}){StreamConfig graphx;graphx.profile=Profile::graphx_radio;graphx.maximum_samples_per_packet=expected;graphx.ip_mtu=9000;auto actual=packet_samples(graphx);if(!actual||*actual!=expected)return 8;}
 StreamConfig excessive;excessive.maximum_samples_per_packet=1025;if(packet_samples(excessive))return 9;
 StreamConfig legacy;legacy.maximum_samples_per_packet=257;if(packet_samples(legacy))return 10;
 return 0;}
