...
int main(){
 ...
 for(std::size_t expected:{1u,2u,1023u,1024u}){StreamConfig sdr;sdr.profile=Profile::sdr_radio;sdr.maximum_samples_per_packet=expected;sdr.ip_mtu=9000;auto actual=packet_samples(sdr);if(!actual||*actual!=expected)return 8;}
