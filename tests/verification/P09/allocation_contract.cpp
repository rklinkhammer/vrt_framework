...
int main(){AdmissionPool pool(AdmissionPool::reference_capacities());RevisionStore<4> store;Consumer consumer;ContextPublisher<4> publisher(store,{&consumer,Consumer::send,Consumer::data});auto sink=store.binding();EffectiveEvent e;e.association_generation=1;e.time_known=e.ordinal_known=true;e.actual_time={100,0};for(auto&f:e.state.fields)f.validity=Validity::known;e.state.fields[1].value=*Hertz::from_integer(1);e.state.fields[2].value=std::uint32_t{valid_data_enable|valid_data_indicator};e.state.fields[3].value=PayloadFormat{0x200003cf00000000ULL};
