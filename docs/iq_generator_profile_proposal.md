...
Accepted cancellation retry policy (D-P07-1): one immutable cancellation meaning per original transaction identity over its active and retained lifetime. The initial request may select a subset; identical retries attach/replay, while later requests with different meaning are rejected without additional cancellation effects. Ordinary and cancellation results are retained separately under header L, including their original state observations. See framework architecture §6 for joint retention and conflict handling.
