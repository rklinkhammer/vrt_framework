# GraphX radio profile

`profiles::iq::Profile::graphx_radio` is an explicit opt-in profile. Existing
`generator_v1` and `frequency_tunable` wire identities and command policies remain
separate. The [Release-baseline audit](graphx-migration-audit.md) identifies the
...
ReqX+ReqS. This document describes library support; GraphX's executable/Soapy/mTLS
...
are inferred locally only when the GraphX receiver profile is explicitly selected.
...
adapter; GraphX must supply its own SourceProvider for the same device it controls.
...
clock binding and a declared runtime OUI (`profiles::iq::graphx_unknown_oui` is
...
```cpp
vita::GraphxRadioSettings settings;
...
cannot bypass the GraphX wire configuration/start lifecycle.
...
actual effective time with `time_known=true` for GraphX. Keep cross-setting
...
GraphX should instantiate a control ingress maximum 1024 bytes, input/output queues
...
state has 8 slots, execution plans allow 4 settings and GraphX status allows 5 query
...
configuration consistent with GraphX's 256 replay-entry requirement and number of
radio instances. Nonzero increasing message IDs are enforced for GraphX; retained
...
target_link_libraries(graphx_radio PRIVATE vita::core)
# Optional supplied UDP adapter:
# set(VITA_BUILD_POSIX_UDP ON CACHE BOOL "" FORCE) before add_subdirectory
# target_link_libraries(graphx_radio PRIVATE vita::posix_udp)
```

Use C++23 with the framework's no-exception/no-RTTI target requirements. The GraphX
...
bindings. The accompanying GraphX design changes select that migration; they do
...
|---|---|---|
| Atomic four-setting operation, no partial effects | `p17_atomic`, `p17_graphx_runtime`; Backend batch commit | Soapy device atomic commit adapter and hardware failure qualification |
| AckV admission, terminal AckX, X-only, X+S | `p17_graphx_runtime`, existing P06/P07/P16 transaction gates | Controller requests and independent cross-process checks |
