...
constexpr bool sdr_capability_field(FieldId selector) noexcept {
  return selector == RFReferenceFrequency::id || selector == SampleRate::id ||
         selector == Bandwidth::id || selector == Gain::id;
}
template <class Packet>
inline Result<void> populate_sdr_capability_response(
    Packet &response, const SdrCapabilities &capabilities,
...
sdr_capability_response(const SdrCapabilities &capabilities,
                           std::span<const FieldId> selectors) noexcept {
  ...
  auto attributed =
      populate_sdr_capability_response(response, capabilities, selectors);
  if (!attributed)
